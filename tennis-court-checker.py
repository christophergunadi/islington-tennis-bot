    ...
    def check_availability(self, date):
        try:
            ...
            response = requests.get(
                f"{self.base_url}?date={date}", headers=self.headers
            )
            ...
            for days_ahead in range(6):
                ...
                try:
                    ...
                    available_slots = await loop.run_in_executor(
                        None, self.check_availability, check_date
                    )
                    current_available_slots = {
                        self.format_timerange(slot) for slot in available_slots
                    }
                    unavailable_slots = (
                        self.previous_available_slots[check_date]
                        - current_available_slots
                    )
                    new_slots = [
                        slot
                    ...
                    for slot in unavailable_slots:
                        self.previous_available_slots[check_date].remove(
                            self.format_timerange(slot)
                        )
